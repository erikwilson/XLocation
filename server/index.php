<?php
	...
	require_once('xlocation.inc.php');
	...
	function log_error($message, $my_email, $data = null) {
		error_log(
			'XLocation message=' . $message . PHP_EOL . PHP_EOL .
	...
	if (!empty($format) && $format == 'json') {
		...
		// Check XLocation version
		if (empty($data->xlocation_version) || (int)$data->xlocation_version < 219) {
			header('Content-Type: application/json');
			echo json_encode(array('ok' => false, 'errno' => 102, 'error' => 'Please upgrade to at least XLocation version 1.11'));
			exit();
		}
		...
		if (empty($action) || $action == 'submit') {
			...
			foreach ($data->application_name as $application_name)
				for ($i = 0; $i < count($data->package_name); $i++) {
					$sql = "INSERT INTO xlocation_app (application_name, package_name, package_version, package_version_code) VALUES ";
			...
			for ($i = 0; $i < count($data->package_name); $i++)
				foreach ($data->settings as $restriction) {
					...
					$sql = "INSERT INTO xlocation (android_id_md5, android_sdk, xlocation_version,";
					...
					$sql .= "," . (empty($data->xlocation_version) ? 'NULL' : (int)$data->xlocation_version) . "";
					$sql .= ",'" . $db->real_escape_string($data->package_name[$i]) . "'";
					$sql .= ",'" . ($i < count($data->package_version_name) ? $db->real_escape_string($data->package_version_name[$i]) : '') . "'";
					$sql .= "," . ($i < count($data->package_version_code) ? (int)$data->package_version_code[$i] : 0);
					...
					$sql .= " ON DUPLICATE KEY UPDATE";
					$sql .= " xlocation_version=VALUES(xlocation_version)";
		...
		// Fetch settings
			...
			// Get restrictions
			$settings = Array();
			$empty = ($data->xlocation_version >= 240);
		...
		// Update
			...
			if (empty($data->android_id))
				$data->android_id = '';
			if (empty($data->xlocation_version))
				$data->xlocation_version = 0;

			// Throttling
			if ($data->xlocation_version >= 395) { // 2.99.29 (395) / 2.2.9 (397)
				$sql = "SELECT COUNT(*) AS count FROM xlocation_update";
				$sql .= " WHERE android_id_md5 = '" . $db->real_escape_string($data->android_id) . "'";
				$sql .= " AND time > '" . date('Y-m-d H:i:s', time() - 12 * 3600) . "'";
				$sql .= " AND installed_version = '" . $db->real_escape_string($data->xlocation_version_name) . "'";
			...
			$latest = null;
			$modified = null;
			$files = glob($folder . '/XLocation_*.apk');
			if ($files)
			...
			// Register check
			$sql = "INSERT INTO xlocation_update (android_id_md5, installed_version, test_versions, current_version)";
			$sql .= " VALUES (";
			$sql .= "'" . $db->real_escape_string($data->android_id) . "'";
			$sql .= ", '" . $db->real_escape_string($data->xlocation_version_name) . "'";
			...
			// Check if newer
			if ($latest == null || version_compare($latest, $data->xlocation_version_name) <= 0)
				header($_SERVER['SERVER_PROTOCOL'] . ' 204 No Content');
			else {
				// Send latest
				$apk = $folder . '/XLocation_' . $latest . '.apk';
...
<html>
	<head>
		<title>XLocation</title>
		<meta charset="utf-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<meta name="description" content="XLocation">
			...
			<div class="container">
				...
				<div class="navbar-collapse collapse">
					<ul class="nav navbar-nav">
						<li><a href="https://github.com/M66B/XLocation#xlocation" target="_blank">XLocation</a></li>
						<li><a href="http://www.xlocation.eu/" target="_blank">Pro license</a></li>
					</ul>
...
<?php
					for ($i = 0; $i < count($package_names); $i++) {
						echo '<a href="/xlocation?package_name=' . urlencode($package_names[$i]) . '">';
...
?>
			...
			<div class="page-header">
				<p>This is a voting system for
					<a href="https://github.com/M66B/XLocation#xlocation">XLocation</a> restrictions.<br />
					Everybody using XLocation can submit his/her restriction settings.<br />
					With a <a href="http://www.xlocation.eu/">Pro license</a> you can fetch submitted restriction settings.<br />
...
?>
					<div class="page-header">
						<p>
							...
							<a class="action" href="http://forum.faircode.eu/forums/forum/android/xlocation/applications/?package_name=<?php echo urlencode($package_name); ?>" target="_blank">Discussion</a>
					...
					if (empty($package_name)) {
						...
						if ($result) {
							while (($row = $result->fetch_object())) {
								...
								echo '<td>' . ($row->method ? '' :
								'<a href="https://github.com/M66B/XLocation#' . urlencode($row->restriction) . '" target="_blank">' .
